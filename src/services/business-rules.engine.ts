...
export class BusinessRulesEngine {
  ...
  async validateBooking(
    ...
    try {
      ...
      const timingResult = await this.validateTimingRules(criteria, slotData, context);
      this.mergeResults(result, timingResult);

      // Validate patient eligibility
      const patientResult = await this.validatePatientEligibility(criteria, context);
      this.mergeResults(result, patientResult);

      // Validate doctor availability
      const doctorResult = await this.validateDoctorAvailability(criteria, slotData, context);
      this.mergeResults(result, doctorResult);

      // Validate specialty-specific rules
      const specialtyResult = await this.validateSpecialtyRules(criteria, context);
      this.mergeResults(result, specialtyResult);

      // Validate appointment type constraints
      const typeResult = await this.validateAppointmentTypeConstraints(criteria, context);
      this.mergeResults(result, typeResult);

      // Validate resource availability
      const resourceResult = await this.validateResourceAvailability(criteria, slotData, context);
      this.mergeResults(result, resourceResult);

      // Check for emergency overrides
      if (criteria.isEmergency) {
        const emergencyResult = await this.applyEmergencyOverrides(result, criteria, context);
        this.mergeResults(result, emergencyResult);
      }

      result.isValid = result.violations.filter(v => v.severity === 'ERROR').length === 0;
  ...
  async validateCancellation(
    ...
    try {
      ...
      if (patientHistory.recentNoShows >= NO_SHOW_POLICY.STRIKES_BEFORE_SUSPENSION - 1) {
        result.warnings.push({
          rule: 'NO_SHOW_RISK',
  ...
  async validateRescheduling(
    ...
    try {
      ...
      if (appointment.rescheduleCount >= RESCHEDULING_RULES.MAX_RESCHEDULES_PER_APPOINTMENT) {
        result.violations.push({
          rule: 'MAX_RESCHEDULES',
          severity: 'ERROR',
          message: 'Maximum reschedule limit reached',
          code: 'RESCHEDULE_LIMIT_EXCEEDED',
          details: { limit: RESCHEDULING_RULES.MAX_RESCHEDULES_PER_APPOINTMENT }
        });
      }

      // Check minimum notice period
      if (!BusinessRules.canReschedule(appointment.rescheduleCount, hoursBeforeOriginal)) {
        result.violations.push({
      ...
      if (!RESCHEDULING_RULES.SAME_DAY_RESCHEDULE_ALLOWED && 
          hoursBeforeNew < 24 && hoursBeforeOriginal < 24) {
        result.violations.push({
      ...
      if (hoursBeforeOriginal < rescheduleFreeCutoff) {
        const fee = appointment.fee?.toNumber() ? appointment.fee.toNumber() * 0.2 : 0; // 20% fee
        result.modifications.push({
  ...
  async applyEmergencyOverrides(
    ...
    if (criteria.urgencyLevel && criteria.urgencyLevel >= EMERGENCY_RULES.PRIORITY_SCORE_THRESHOLD) {
      // Override capacity restrictions
      const capacityViolations = currentResult.violations.filter(v => 
        v.code.includes('CAPACITY') || v.code.includes('OVERBOOKING')
      );

      if (capacityViolations.length > 0 && EMERGENCY_RULES.AUTO_OVERRIDE_CAPACITY) {
        result.warnings.push({
  ...
  async validatePatientEligibility(
    ...
    try {
      ...
      // Check no-show history
      const noShowCount = patient.appointments.filter(a => a.status === AppointmentStatus.NO_SHOW).length;
      if (noShowCount >= NO_SHOW_POLICY.STRIKES_BEFORE_SUSPENSION) {
        result.violations.push({
          ...
          details: { noShowCount, limit: NO_SHOW_POLICY.STRIKES_BEFORE_SUSPENSION }
        });
      ...
      if (patient.insurance) {
        const insuranceValidation = await this.validateInsuranceCoverage(patient.insurance, criteria);
  ...
  private async validateTimingRules(
    ...
    if (!BusinessRules.isBusinessHour(appointmentTime.toTimeString().slice(0, 5))) {
      result.violations.push({
        rule: 'BUSINESS_HOURS',
        severity: 'ERROR',
  ...
  private async validateDoctorAvailability(
    ...
    if (criteria.doctorId) {
      ...
      if (!doctor.acceptsNewPatients && context.patient?.appointments?.length === 0) {
        result.violations.push({
          rule: 'NEW_PATIENTS',
          severity: 'ERROR',
  ...
  private async getPatientNoShowHistory(patientId: string): Promise<{ recentNoShows: number }> {
    const { prisma } = this.deps;
  ...
  private mergeResults(target: BusinessRuleResult, source: BusinessRuleResult): void {
