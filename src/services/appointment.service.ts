...
export class AppointmentService {
  ...
  async bookAppointment(
    ...
    const { prisma, logger, businessRulesEngine, availabilityService } =
      this.deps;
    ...
    try {
      ...
      const ruleValidation = await businessRulesEngine.validateBooking(
        criteria,
        slot,
      );

      if (!ruleValidation.isValid) {
        const criticalErrors = ruleValidation.violations.filter(
          v => v.severity === 'ERROR',
        );
        throw new Error(
          `Booking validation failed: ${criticalErrors.map(e => e.message).join(', ')}`,
        );
      }
      ...
      try {
        ...
        const updatedAppointment = await this.applyRuleModifications(
          appointment,
          ruleValidation,
        );
        ...
        await this.processWaitingQueue(
          updatedAppointment.specialtyId,
          updatedAppointment.doctorId,
        );
        ...
        return {
          ...
          suggestedImprovements: ruleValidation.modifications.map(
            m => m.reason,
          ),
  ...
  async cancelAppointment(
    ...
    const { prisma, logger, businessRulesEngine, coreSchedulingService } =
      this.deps;
    ...
    try {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Cancellation not allowed: ${validation.violations.map(v => v.message).join(', ')}`,
        );
      }
      ...
      await this.scheduleCancellationNotifications(
  ...
  async rescheduleAppointment(
    ...
    try {
      ...
      if (!validation.isValid) {
        throw new Error(
          `Rescheduling not allowed: ${validation.violations.map(v => v.message).join(', ')}`,
        );
      }
      ...
      try {
        ...
        const fee = this.calculateReschedulingFee(
          currentAppointment,
        ...
        await this.scheduleReschedulingNotifications(
          currentAppointment,
  ...
  async getAppointmentById(
    ...
    try {
      ...
      this.deps.logger.error('Error getting appointment', {
        error,
        appointmentId,
      });
      throw error;
  ...
  async searchAppointments(
    ...
    try {
      ...
      this.deps.logger.error('Error searching appointments', {
        error,
        filters,
      });
      throw error;
  ...
  async getAppointmentHistory(
    ...
    try {
      ...
      this.deps.logger.error('Error getting appointment history', {
        error,
        patientId,
      });
      throw error;
  ...
  private async getAndValidateSlot(
    slotId: string,
  ...
  private async findReschedulingSuggestions(
    appointment: Appointment,
  ): Promise<AvailableSlot[]> {
  ...
  private async scheduleAppointmentNotifications(
    ...
    this.deps.logger.info('Appointment notifications scheduled', {
      appointmentId: appointment.id,
    });
  }

  private async scheduleCancellationNotifications(
    appointment: Appointment,
    fee: number,
  ): Promise<void> {
    // Schedule cancellation notifications
    this.deps.logger.info('Cancellation notifications scheduled', {
      appointmentId: appointment.id,
      fee,
    });
  }

  private async scheduleReschedulingNotifications(
  ...
  private async scheduleCompletionNotifications(
    appointment: Appointment,
  ): Promise<void> {
    // Schedule completion and follow-up notifications
    this.deps.logger.info('Completion notifications scheduled', {
      appointmentId: appointment.id,
    });
  }
  ...
  private async processWaitingQueue(
    ...
    this.deps.logger.info('Processing waiting queue', {
      specialtyId,
      doctorId,
    });
  }

  private async processWaitingQueueForSlot(
    appointment: Appointment,
  ): Promise<void> {
  ...
  private async updatePatientMedicalHistory(
    ...
    this.deps.logger.info('Updating patient medical history', {
      appointmentId: appointment.id,
    });
  }
}
