...
export class N8NWorkflowManager {
  ...
  async createWorkflow(workflow: WorkflowDefinition): Promise<WorkflowDefinition> {
    try {
      ...
      logger.info('Workflow created successfully', { workflowId: response.data.id });
      return response.data;
    } catch (error) {
      logger.error('Failed to create workflow', { workflowName: workflow.name, error });
   ...
   */
  async updateWorkflow(workflowId: string, workflow: Partial<WorkflowDefinition>): Promise<WorkflowDefinition> {
    try {
      const response = await this.client.put(`/api/v1/workflows/${workflowId}`, workflow);
   ...
   * List all workflows
   */
  async listWorkflows(active?: boolean, tags?: string[]): Promise<WorkflowDefinition[]> {
    try {
      const params: any = {};
  ...
  async setWorkflowActive(workflowId: string, active: boolean): Promise<void> {
    try {
      await this.client.patch(`/api/v1/workflows/${workflowId}/activate`, { active });
      logger.info('Workflow activation status changed', { workflowId, active });
    } catch (error) {
      logger.error('Failed to change workflow activation status', { workflowId, active, error });
      throw new Error(`Failed to ${active ? 'activate' : 'deactivate'} workflow: ${error}`);
    }
  }
  ...
  async executeWorkflow(workflowId: string, inputData?: Record<string, any>): Promise<WorkflowExecution> {
    try {
      ...
      logger.info('Workflow executed successfully', { workflowId, executionId: response.data.id });
      return response.data;
    } catch (error) {
   ...
   */
  async getExecution(executionId: string, includeData: boolean = false): Promise<WorkflowExecution> {
    try {
      const params = includeData ? { includeData: 'true' } : {};
      const response = await this.client.get(`/api/v1/executions/${executionId}`, { params });
      return response.data;
    } catch (error) {
  ...
  async getWorkflowStats(workflowId: string): Promise<any> {
    try {
      const response = await this.client.get(`/api/v1/workflows/${workflowId}/statistics`);
      return response.data;
    } catch (error) {
   ...
   */
  async bulkActivateWorkflows(workflowIds: string[], active: boolean): Promise<void> {
    const promises = workflowIds.map(id => this.setWorkflowActive(id, active));
    await Promise.allSettled(promises);
    logger.info('Bulk workflow activation completed', { count: workflowIds.length, active });
  }
   ...
   */
  async importWorkflow(workflowData: WorkflowDefinition): Promise<WorkflowDefinition> {
   ...
   */
  async cloneWorkflow(workflowId: string, newName: string): Promise<WorkflowDefinition> {
    try {
