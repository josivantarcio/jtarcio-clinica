...
export class N8NDeploymentManager {
   ...
   */
  async deployComplete(config: DeploymentConfig = {
   ...
   */
  private async validateN8NConnection(): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  private async deployWorkflows(config: DeploymentConfig): Promise<{
    ...
    try {
      ...
      const highPriorityWorkflows = workflowTemplateManager.getHighPriorityWorkflows();
      ...
      for (const template of highPriorityWorkflows) {
        try {
          ...
          if (existingWorkflow && !config.force) {
            logger.info('Workflow already exists, skipping', { workflowId: template.id });
          ...
          const deployedWorkflow = await workflowTemplateManager.deployWorkflow(template);
          ...
          logger.error('Failed to deploy workflow', { workflowId: template.id, error });
  ...
  private async setupCredentials(config: DeploymentConfig): Promise<void> {
    logger.info('Setting up credentials (placeholder - would configure actual credentials)');
    ...
    logger.warn('Credentials must be configured manually in N8N interface', {
      ...
      private async initializeMonitoring(config: DeploymentConfig): Promise<void> {
        ...
        try {
          ...
          const monitoringWorkflow = workflows.find(w => w.name === 'monitoring-sistema');
    
          if (monitoringWorkflow && monitoringWorkflow.active) {
            logger.info('Monitoring workflow is active', { workflowId: monitoringWorkflow.id });
            
            // Execute initial health check
            const healthCheck = await workflowManager.executeWorkflow(monitoringWorkflow.id!, {
              initialCheck: true,
              environment: config.environment
            });
    
            logger.info('Initial health check executed', { executionId: healthCheck.id });
          }
    
          // Start metrics workflow
          const metricsWorkflow = workflows.find(w => w.name === 'business-metrics');
          if (metricsWorkflow && metricsWorkflow.active) {
            logger.info('Business metrics workflow is active', { workflowId: metricsWorkflow.id });
          }
      ...
      private async runTestExecutions(config: DeploymentConfig): Promise<void> {
        ...
        try {
          ...
          const testableWorkflows = workflows.filter(w => 
            !w.name.includes('monitoring') && !w.name.includes('metrics')
          );
    
          for (const workflow of testableWorkflows.slice(0, 3)) { // Test first 3 workflows
            try {
              logger.info('Testing workflow', { workflowId: workflow.id, name: workflow.name });
              
              const testData = this.generateTestData(workflow.name);
              const execution = await workflowManager.executeWorkflow(workflow.id!, testData);
      ...
      async validateDeployment(): Promise<{
        ...
        try {
          ...
          const criticalWorkflows = ['novo-agendamento', 'sistema-lembretes', 'monitoring-sistema'];
          const inactiveCritical = healthCheck.workflows.filter(w => 
            criticalWorkflows.includes(w.name) && !w.active
          );
    
          if (inactiveCritical.length > 0) {
            issues.push(`Critical workflows inactive: ${inactiveCritical.map(w => w.name).join(', ')}`);
          }
    
          // Check for workflows with errors
          const errorWorkflows = healthCheck.workflows.filter(w => w.status === 'error');
          if (errorWorkflows.length > 0) {
            issues.push(`Workflows with errors: ${errorWorkflows.map(w => w.name).join(', ')}`);
          }
      ...
      async cleanup(): Promise<void> {
        ...
        try {
          ...
          const testExecutions = executions.filter(e => 
            e.data?.resultData?.startData?.test === true
          );
    
          for (const execution of testExecutions) {
            try {
              await workflowManager.deleteExecution(execution.id);
              logger.info('Test execution cleaned up', { executionId: execution.id });
            } catch (deleteError) {
