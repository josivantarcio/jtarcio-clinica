...
export class N8NErrorHandler {
  ...
  async handleWorkflowError(
    ...
    try {
      ...
      const workflowError = await this.parseWorkflowError(
        workflowId,
        executionId,
        error,
        context,
      ...
      const recoveryActions = await this.determineRecoveryActions(
        workflowError,
        config,
      );
  ...
  private async parseWorkflowError(
    ...
    try {
      ...
      else if (error.status >= 500 || error.message?.includes('API'))
  ...
  private async determineRecoveryActions(
    ...
    if (
      ...
      error.retryCount >= Math.ceil(config.maxRetries / 2)
    ) {
      actions.push({
        type: 'fallback',
    ...
    try {
      switch (action.type) {
        case 'retry':
          ...
          return {
            success: true,
            message: `Retry successful: ${retryResult.id}`,
          };
        ...
        case 'fallback':
          if (!config.fallbackWorkflow) {
            return {
              success: false,
              message: 'No fallback workflow configured',
            };
          }
          ...
          return {
            success: true,
            message: `Fallback executed: ${fallbackResult.id}`,
          };
        ...
        case 'restart':
          ...
          return {
            success: true,
            message: `Workflow restarted: ${restartResult.id}`,
          };
        ...
        default:
          return {
            success: false,
            message: `Unknown recovery action: ${action.type}`,
          };
      }
    } catch (actionError) {
  ...
  private async sendErrorAlert(error: WorkflowError): Promise<void> {
    try {
      ...
      const errorAlertWorkflow = alertWorkflow.find(
        w => w.name === 'error-alert-notification',
      );
  ...
  private async escalateError(
    ...
    try {
      logger.error('Error escalated to administrators', {
        ...
        failedActions: actions.filter(
          a => a.executed && !a.result?.includes('successful'),
        ).length,
      });
  ...
  private getErrorConfig(workflowName: string): ErrorConfig {
    ...
    if (
      workflowName.includes('agendamento') ||
      workflowName.includes('payment')
    ) {
      category = 'critical';
    } else if (
      workflowName.includes('lembrete') ||
      workflowName.includes('notification') ||
      workflowName.includes('whatsapp') ||
      workflowName.includes('sms')
    ) {
      category = 'communication';
    } else if (
      workflowName.includes('monitoring') ||
      workflowName.includes('health')
    ) {
      category = 'monitoring';
    } else if (
      workflowName.includes('metrics') ||
      workflowName.includes('report')
    ) {
      category = 'reporting';
    }
  ...
  private isRetryable(error: WorkflowError): boolean {
    ...
    if (
      nonRetryableMessages.some(msg =>
        error.errorMessage.toLowerCase().includes(msg),
      )
    ) {
      return false;
    }
  ...
  getErrorStatistics(): {
    ...
    return {
      ...
      errorsByType: this.errorHistory.reduce(
        (acc, error) => {
      ...
      errorsByWorkflow: this.errorHistory.reduce(
        (acc, error) => {
