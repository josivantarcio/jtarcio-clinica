...
export async function availabilityRoutes(
  ...
  fastify.post(
    ...
    async (
      request: FastifyRequest<{ Params: { doctorId: string }; Body: any }>,
      reply: FastifyReply,
    ) => {
    ...
    async (
      ...
      return reply.status(501).send({
        ...
        error: {
          code: 'NOT_IMPLEMENTED',
          message:
            'Get doctor availability settings endpoint not yet implemented',
        },
      });
    },
  );
