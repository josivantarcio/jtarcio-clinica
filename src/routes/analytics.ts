...
export async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/analytics', {
    ...
    try {
      ...
      const revenueGrowth = previousRevenue._sum.fee 
        ? ((Number(totalRevenue._sum.fee || 0) - Number(previousRevenue._sum.fee)) / Number(previousRevenue._sum.fee)) * 100
        : 0
      ...
      const conversionRate = totalScheduled > 0 ? (completedAppointments / totalScheduled) * 100 : 0
      const cancelRate = totalScheduled > 0 ? (cancelledAppointments / totalScheduled) * 100 : 0
      ...
      const avgDailyRevenue = periodDuration > 0 
        ? Number(totalRevenue._sum.fee || 0) / (periodDuration / (24 * 60 * 60 * 1000))
        : 0

      const avgDailyAppointments = periodDuration > 0 
        ? totalAppointments / (periodDuration / (24 * 60 * 60 * 1000))
        : 0

      const nextMonthRevenue = avgDailyRevenue * 30 * (1 + revenueGrowth / 100)
      const nextMonthAppointments = Math.round(avgDailyAppointments * 30 * (1 + appointmentGrowth / 100))

      // Capacity calculation (assuming 8 hours/day, 30 min/appointment)
      const maxDailyCapacity = 16 // 8 hours * 2 appointments per hour
      const currentCapacity = avgDailyAppointments > 0 ? (avgDailyAppointments / maxDailyCapacity) * 100 : 0
      ...
      const analyticsData: AnalyticsResponse = {
        ...
        advanced: {
          ...
          operationalEfficiency: Math.max(0, 100 - Number(cancelRate.toFixed(1)))
        },
        predictions: {
  ...
  fastify.get('/analytics/revenue-chart', {
    preHandler: [verifyJWT],
    schema: {
